...
while response.lower().replace(" ", "") not in exits:
    ...
    # print(parse)
    query = intent.build_query_for(intent.get_associated_properties(parser.entity_extraction_tokenizer(response)))
    #print(intent.get_associated_properties(parser.entity_extraction_tokenizer(response)))
    #print(query)
    rows = g.query(query)
    #print(len(rows))
