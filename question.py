...
class QuestionParser:
    ...
    def entity_extraction_tokenizer(self, text):
        ...
        for word in words:
            ...
            if len(word) > 0 and word not in km.STOP_WORDS:
                tokens.append((km.nlp(word)[0].lemma_))
        # print(tokens)
        return tokens
    ...
    def parse_question(self, question_string):
        ...
        features = []
        try:
            tokens = km.nlp(question_string)
            # words = nltk.word_tokenize(original_string)
            # tagged = nltk.pos_tag(words)
            # print(tagged)
            features = [(token.lemma_, km.nlp(token.lemma_).vector_norm, token.pos_) for token in tokens]
            print(features)
